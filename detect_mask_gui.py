...
# ----------------------------------------------- PREDICT EMOTION ---------------------------------------------------
def get_eyes(face, rect):
    # identificazione landmarks facciali sull' immagine in input
    predictor = dlib.shape_predictor("face_detector/shape_predictor_68_face_landmarks.dat")
    shape = predictor(face, rect)

    # ritaglio della porzione di occhi e fronte
...
def predict_emotion_mask(face):
    labels = ["angry", "disgust", "fear", "happy", "neutral", "sad", "surprise"]

    # preprocessing dell'immagine in input
    face = img_to_array(face)
    face = face.reshape((1, face.shape[0], face.shape[1], face.shape[2]))

    # caricamento modello
    model = load_model("models/cropped_face.h5")

    # predizione emozione
    predictions = model.predict(face)[0]
    pred = np.argmax(predictions)
    return labels[pred], predictions[pred]

def predict_emotion_without_mask(face):
    labels = ["angry", "disgust", "fear", "happy", "neutral", "sad", "surprise"]
    ...
    # caricamento modello
    model = load_model("models/full_face.h5")
...
def select_image():
    ...
    global path, face, imgOriginalScene, h, w
    path = tkFileDialog.askopenfilename()
    if len(path) > 0:
        print(path)
        imgOriginalScene = cv2.imread(path)
        face = cv2.resize(imgOriginalScene, (400, 300))
        face = cv2.cvtColor(face, cv2.COLOR_BGR2RGB)
        face = Image.fromarray(face)
        face = ImageTk.PhotoImage(face)
        Label_6.configure(image=face)
...
def predict_face():
    ...
    if path == "":
        ...
        # ottenere la face detection
        text = Label_7.cget("text") + "Analisi del volto...\n"
        Label_7.configure(text=text)
        rects = detector(imgOriginalScene, 1)
        ...
        for (a, rect) in enumerate(rects):
            ...
            # determinare l'etichetta delle emozioni
            emotion_eyes = get_eyes(temp, rect)
            if mask > withoutMask and emotion_eyes.size:
                # calcolo su porzione degli occhi
                emotion_eyes = cv2.resize(emotion_eyes, (48, 48))
                (emotion, prob) = predict_emotion_mask(emotion_eyes)
                print(emotion, prob*100)
            else:
                # calcolo su intero volto
                emotion_face = cv2.resize(temp, (48, 48))
                (emotion, prob) = predict_emotion_without_mask(emotion_face)
                print(emotion, prob*100)

            # includere la probabilità nel modello
            label = "{}: {:.2f}%".format(label, max(mask, withoutMask) * 100)
            emotion_res = "{}: {:.2f}%".format(emotion, prob*100)

            # visualizza l'etichetta e il rettangolo
            cv2.putText(imgOriginalScene, label, (startX, startY - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, color, 2)
            cv2.putText(imgOriginalScene, emotion_res, (startX, endY + 15),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, color, 2)
            cv2.rectangle(imgOriginalScene, (startX, startY), (endX, endY), color, 2)
            find = True

        if not find:
            tkinter.messagebox.showwarning("VOLTI NON RICONOSCIUTI",
                                           "Attenzione: \nIn alcuni casi il detector di \nDLIB non riconosce i volti. \nTestare con un'altra immagine...")
...
def face_recognition():
    ...
    for (box, pred) in zip(locs, preds):
        ...
        #costruire il rettangolo dlib
        rect = dlib.rectangle(startX, startY, endX, endY)

        # determinare l'etichetta delle emozioni
        emotion_face = frame[startY:endY, startX:endX]
        emotion_eyes = get_eyes(emotion_face, rect)
        if mask > withoutMask and emotion_eyes.size:
            # calcolo su porzione degli occhi
            emotion_eyes = cv2.resize(emotion_eyes, (48, 48))
            (emotion, prob) = predict_emotion_mask(emotion_eyes)
            ...
            # calcolo su intero volto
            emotion_face = cv2.resize(emotion_face, (48, 48))
            (emotion, prob) = predict_emotion_without_mask(emotion_face)
        ...
        cv2.putText(frame, label, (startX, startY - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, color, 2)
        cv2.putText(frame, emotion_res, (startX, endY + 15),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.45, color, 2)
...
Label_3 = Button(tab1, font=('arial', 10, 'bold'), image=photobtn, text="ANALIZZA VOLTI", compound="center",
